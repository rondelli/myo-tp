...
$F_{ij}$ binaria: $1$ indica si el archivo $i$ está en el disco $j$, $\forall i = \{1, \ldots, n\},\ \forall j = \{1, \ldots, m\}$, donde $F_{ij} \in {0, 1}$, $0$ en el caso de que no esté\\

$D_{j}$ binaria: $1$ indica que es está usando el disco $j$.

\begin{alignat*}{2}
	\minimize
	& \sum_{j = 1}^{m} D_{j}\\
	%\maximize
	%& \sum_{i = 1}^{n}\sum_{j = 1}^{m} F_{ij}\\
	\subjto
	& \sum_{j = 1}^{m} F_{ij} = 1, \ \forall i \in \{1, \ldots, n\} \quad\text{(el archivo $i$ está en un solo disco)}\\
	& \sum_{i = 1}^{n} f_{i} F_{ij} \le d\cdot D_{j}, \ \forall j \in \{1, \ldots, m\} \quad\text{(archivos que entran en el disco $j$)}\\
	& F_{ij} \le D_{j}M \qquad\text{(no se puede usar un disco vacío)}\\
	& F_{ij} \in \{0,1\}, \quad \forall i \in \{1, \ldots, n\}, \ \forall j \in \{1, \ldots, m\}\\
	& D_{j} \in \{0,1\}, \quad \forall j \in \{1, \ldots, m\}
\end{alignat*}
