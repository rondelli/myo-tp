...
Inicialmente, planteamos el programa lineal de la
Sección~\ref{modelo:primera:parte} con una tercera restricción:
\begin{alignat*}{2}
    \sum_{i = 1}^{n} x_{ij} \le n\cdot y_{j} \quad \forall j \in \{1,
	\ldots, m\},\ \text{\emph{(no se puede usar un disco vacío)}}
\end{alignat*}
En esta sección, explicamos porqué esta restricción no es necesaria,
...
\begin{enumerate}
    \item $0 \leq d \cdot 1$, \text{ya que el disco está vacío (disco 1)}
    \item $s_{1} + s_{2} \leq d \cdot 1$, \text{por hipótesis (disco 2)}
\end{enumerate}
...
\begin{enumerate}
    \item $0 \leq d \cdot 0$, \text{ya que no se elige el disco 1}
    \item $s_{1} + s_{2} \leq d \cdot 1$, \text{por hipótesis (disco 2)}
\end{enumerate}
